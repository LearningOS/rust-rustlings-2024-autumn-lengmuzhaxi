...
extern "Rust" {
    // 这两个函数的声明不需要 #[no_mangle]
    fn my_demo_function(a: u32) -> u32;
    fn my_demo_function_alias(a: u32) -> u32; // 保留这一行作为别名
}
...
mod foo {
    ...
    // 定义 my_demo_function_alias 为 my_demo_function 的别名
    #[no_mangle] // 添加这个属性
...
mod tests {
    ...
    fn test_success() {
        // 外部导入的函数默认是 UNSAFE 的
        unsafe {
            assert_eq!(my_demo_function(123), 123); // 测试 my_demo_function
            assert_eq!(my_demo_function_alias(456), 456); // 测试 my_demo_function_alias
        }
    }
}
